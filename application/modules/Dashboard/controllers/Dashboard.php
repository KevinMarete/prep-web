...
class Dashboard extends MX_Controller {
    ...
    public function get_data($chartname, $filters) {
        if ($chartname == 'hiv_service_offered_chart') {
            $main_data = $this->Service_delivery_model->get_hiv_service_offered($filters);
        } else if ($chartname == 'creatinine_availability_chart') {
            $main_data = $this->Service_delivery_model->get_creatinine_distribution($filters);
        } else if ($chartname == 'hepatitis_b_availability_chart') {
            $main_data = $this->Service_delivery_model->get_hepatitis_b_distribution($filters);
        } else if ($chartname == 'hepatitis_c_availability_chart') {
            $main_data = $this->Service_delivery_model->get_hepatitis_c_distribution($filters);
        } else if ($chartname == 'facility_population_offered_PrEP_chart') {
            $main_data = $this->Service_delivery_model->get_facility_PrEP_distribution($filters);
        } else if ($chartname == 'lab_services_availability_chart') {
            $main_data = $this->Service_delivery_model->get_lab_services_availability($filters);
        ...
        } else if ($chartname == 'staff_dispensing_PrEP_chart') {
            $main_data = $this->Summary_model->get_staff_dispensing_PrEP($filters);
        } else if ($chartname == 'facility_count_chart') {
            $main_data = $this->Summary_model->get_facility_count($filters);
        } else if ($chartname == 'partners_percentage_support_chart') {
            $main_data = $this->Summary_model->get_partners_percentage_support($filters);
        } else if ($chartname == 'facility_ownership_chart') {
            $main_data = $this->Summary_model->get_facility_ownership($filters);
        } else if ($chartname == 'facility_level_chart') {
            $main_data = $this->Summary_model->get_facility_level($filters);
        } else if ($chartname == 'partner_supported_component_chart') {
            $main_data = $this->Summary_model->get_partner_supported_component($filters);
            ...
            $main_data = $this->Monitoring_evaluation_model->get_training_national_PrEP_ME_tools($filters);
            ...
            $main_data = $this->Monitoring_evaluation_model->get_tools_availability($filters);
        ...
        } else if ($chartname == 'follow_up_method_chart') {
            $main_data = $this->Monitoring_evaluation_model->get_follow_up_method($filters);
        }  else if ($chartname == 'service_providers_training_chart') {
            $main_data = $this->Monitoring_evaluation_model->get_service_providers_training($filters);
        } else if ($chartname == 'PrEP_clients_both_ever_initiated_and_currently_on_care_chart') {
            $main_data = $this->Monitoring_evaluation_model->get_PrEP_clients_both_ever_initiated_and_currently_on_care($filters);
        ...
        } else if ($chartname == 'facility_partner_service_delivery_point_table') {
            $main_data = $this->Commodity_management_model->get_facility_partner_service_delivery_point_table($filters);
        } else if ($chartname == 'gaps_per_county_chart') {
            $main_data = $this->Communication_model->get_county_gap_distribution($filters);
        } else if ($chartname == 'gaps_per_county_table') {
            $main_data = $this->Communication_model->get_county_gap_table($filters);
        }
        return $main_data;
    }
